...
from capa_datos_persona.usuario__dao import UsuarioDAO
...
while opcion != 5:
    ...
    if opcion == 1:
        ...
        usuario = Usuario(username_var, password=password_var)
        usuario_insertado = UsuarioDAO.insertar(usuario)
        log.info(f'Usuario insertado: {usuario_insertado}')
    elif opcion == 3:
        id_usuario_var = int(input('Digite el id del uduario a modificar: '))
        username_var = input('Digite el nombre del usuario a modificar: ')
        password_var = input('Digite la contraseñ del usuario a modificar: ')
